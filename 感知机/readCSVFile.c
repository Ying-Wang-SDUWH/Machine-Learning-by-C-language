...
int ReadCsvData(char* csvFilePath)
{
	...
	else
	{
		...
		//printf("%i行%i列", giNumRow, giNumCol);
...
{
	int i, j;
	double *minmax= (double*)malloc(sizeof(double)*(giNumCol-1)*2);  //第一行作为初始最大值最小值
	memset(minmax, 0, sizeof(double)*(giNumCol - 1) * 2);
	for (int c = 0; c < giNumCol-1; c++)
	{
		minmax[2*c] = giCsvData[c];
		minmax[2*c+1] = giCsvData[c];
	}
	for (int p = 0; p < (giNumCol - 1); p++)     //每一列
	{
...
void coefficients_sgd(double l_rate,int n_epoch)
{
	...
	for (int m = (number*(k - 1)); m < giNumRow; m++)
	{
		...
		if (yhat > 0)
			...
			yhat = 0;
		if (yhat == giCsvData[index*giNumCol - 1])
			correct+=1;
	}
	printf("\n单折正确率%f\n", correct / (giNumRow - number * (k - 1)));
}
...
void  evaluate_algo()
{
	...
	{
		movearray();
		coefficients_sgd(0.01, 50);
		rmse += (correct / (double)(giNumRow - number * (k - 1)));
	}
	rmse = (rmse/ (double)k);
	printf("\n\n平均正确率%f\n", rmse);
}